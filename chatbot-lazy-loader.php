...
function crb_attach_theme_options() {
  Container::make( 'theme_options', __( 'Chatbot Lazy Loader' ) )
       ...
       * Provider Selectoin
       */
      Field::make( 'select', 'chat_provider', __( 'Choose a chatbot provider' ) )
        ->set_options( array(
            'drift' => 'Drift',
            'intercom' => 'Intercom',
            'messenger' => 'Messenger',
       ...
       */
      Field::make( 'text', 'indemandly_username', 'Username' )
        ->set_required( true )
       ...
       */
      Field::make( 'checkbox', 'show_button', __( 'Show placholder button' ) )
        ->set_option_value( 'no' )
       ...
       */
      Field::make( 'color', 'chat_button_color', 'Button Color' )
        ->set_required( true )
        ->set_conditional_logic( 
        array(
          'relation' => 'AND', // Optional, defaults to "AND"
...
if ( get_option('_chat_provider') === 'drift' ) {
  ...
  if ( get_option('_drift_key') != null ) {
    ...
    if (get_option('_show_button') === true) {
...
if ( get_option('_chat_provider') === 'indemandly' ) {
  ...
  if ( get_option('_indemandly_username') != null ) {
